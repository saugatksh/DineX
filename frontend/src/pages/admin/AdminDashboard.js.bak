...
function DailyReportView({ onAddExpense }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(false);
    try {
      const res = await API.get(`/admin/daily-report?date=${selectedDate}`);
      setReport(res.data);
    } catch (err) {
      setLoadError(true);
      setReport(null);
    ...
    <>
      ...
      {!loading && loadError && (
        <div className="alert alert-error" style={{ marginBottom: 16 }}>
          ⚠️ Failed to load report. Please check that the backend is running and try again.
      ...
      {!loading && !loadError && !report && (
        <div className="empty-state"><div className="empty-icon">📅</div><h3>No data found for this date</h3><p style={{ color: "var(--text-muted)", fontSize: 13 }}>No paid orders or expenses were recorded on this day.</p></div>
      )}
    </>
  );
}
...
function TablesTab() {
  ...
  const [loading, setLoading] = useState(true);
  const [form, setForm] = useState({ table_number: "", capacity: 4, table_label: "", table_section: "Main" });
  const [saving, setSaving] = useState(false);
  ...
  // When section changes, auto-generate label preview
  const handleSectionChange = (section) => {
    const countInSection = tables.filter(t =>
      (t.table_section || "Main").toLowerCase() === (section || "Main").toLowerCase()
    ).length;
    const autoLabel = getAutoLabel(section, countInSection);
    setAutoLabelPreview(autoLabel);
    setForm(f => ({ ...f, table_section: section, table_label: autoLabel }));
  };
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setForm({ table_number: "", capacity: 4, table_label: "", table_section: form.table_section });
      setAutoLabelPreview("");
      await load();
    } catch {}
    setSaving(false);
    ...
    <>
      ...
      <div className="page-body">
        <div className="card" style={{ marginBottom: 24 }}>
          ...
          {(() => {
            const sections = [...new Set(tables.map(t => t.table_section || "Main").filter(Boolean))];
            if (!sections.includes("Main")) sections.unshift("Main");
            return null;
          })()}
          <form onSubmit={handleAdd}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
              <div className="form-group" style={{ flex: "1 1 100px", minWidth: 90 }}>
              ...
              <div className="form-group" style={{ flex: "1 1 140px", minWidth: 130 }}>
                ...
                <input
                  ...
                  placeholder="e.g. Garden, Rooftop, Ground Floor"
                  value={form.table_section}
              ...
              <div className="form-group" style={{ flex: "1 1 130px", minWidth: 120 }}>
                <label>
                  Table Label
                  {autoLabelPreview && <span style={{ color: "var(--accent)", fontSize: 11, marginLeft: 6, fontWeight: 400 }}>auto: {autoLabelPreview}</span>}
                </label>
                <input
                  placeholder="e.g. G1, R3, FF2"
                  value={form.table_label}
                  onChange={e => setForm({ ...form, table_label: e.target.value })}
                />
              </div>
              <button type="submit" className="btn btn-primary" disabled={saving} style={{ marginBottom: 1 }}>{saving ? <span className="spinner-sm" /> : "Add Table"}</button>
            </div>
            {autoLabelPreview && form.table_section && form.table_section !== "Main" && (
              <div style={{ marginTop: 8, fontSize: 12, color: "var(--text-muted)", padding: "6px 10px", background: "var(--bg-surface)", borderRadius: 8, border: "1px solid var(--border)" }}>
                💡 Auto-label <strong style={{ color: "var(--accent)" }}>{autoLabelPreview}</strong> generated from section "{form.table_section}" — edit above if needed
              </div>
            )}
          </form>
        </div>
