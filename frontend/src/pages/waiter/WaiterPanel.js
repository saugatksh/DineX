...
@keyframes bounceIn{0%{opacity:0;transform:scale(0.8);}60%{transform:scale(1.07);}100%{opacity:1;transform:scale(1);}}
...
/* Buttons */
.btn{display:inline-flex;align-items:center;gap:7px;font-family:var(--font);font-weight:600;font-size:14px;padding:9px 18px;border-radius:10px;cursor:pointer;border:none;transition:all 0.18s cubic-bezier(0.34,1.56,0.64,1);line-height:1;white-space:nowrap;position:relative;overflow:hidden;}
...
.btn-primary{background:var(--gradient-brand);color:#fff;box-shadow:0 2px 10px rgba(245,158,11,0.35);}
.btn-primary:hover{filter:brightness(1.08);box-shadow:0 6px 20px rgba(245,158,11,0.5);transform:translateY(-1px);}
.btn-secondary{background:var(--bg-elevated);color:var(--text-primary);border:1px solid var(--border-md);}
.btn-secondary:hover{background:var(--bg-hover);border-color:var(--border-focus);transform:translateY(-1px);}
...
.table-tile{background:var(--bg-card);border:1px solid var(--border);border-radius:16px;padding:20px 14px;text-align:center;cursor:pointer;transition:all 0.25s cubic-bezier(0.34,1.3,0.64,1);animation:fadeInUp 0.35s ease both;position:relative;overflow:hidden;}
...
.table-tile:hover{border-color:var(--accent-border);transform:translateY(-5px) scale(1.02);box-shadow:var(--shadow-lg);}
.table-tile:active{transform:translateY(-2px) scale(0.99);}
.table-tile.occupied{border-color:var(--accent-border);background:var(--accent-bg);box-shadow:0 0 0 1px var(--accent-border);}
...
/* Menu item card */
.menu-card{background:var(--bg-card);border:1px solid var(--border);border-radius:14px;padding:14px;text-align:left;cursor:pointer;transition:all 0.2s cubic-bezier(0.34,1.3,0.64,1);font-family:var(--font);color:var(--text-primary);position:relative;overflow:hidden;animation:fadeIn 0.25s ease both;}
.menu-card::after{content:'';position:absolute;inset:0;background:radial-gradient(circle at 50% 50%,rgba(245,158,11,0.12),transparent 70%);opacity:0;transition:opacity 0.2s;}
.menu-card:hover:not(:disabled){border-color:var(--accent-border);transform:translateY(-3px) scale(1.01);box-shadow:var(--shadow-md);}
.menu-card:hover:not(:disabled)::after{opacity:1;}
...
/* Modal */
.modal-overlay{position:fixed;inset:0;background:var(--overlay);display:flex;align-items:center;justify-content:center;z-index:1000;padding:20px;backdrop-filter:blur(8px);animation:fadeIn 0.18s ease;}
.modal{background:var(--bg-card);border:1px solid var(--border-md);border-radius:20px;box-shadow:var(--shadow-lg),0 0 0 1px rgba(255,255,255,0.04);width:100%;max-width:460px;animation:scaleIn 0.22s cubic-bezier(0.34,1.56,0.64,1);overflow:hidden;}
.modal-hdr{padding:20px 22px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;background:linear-gradient(135deg,var(--bg-surface),var(--bg-card));}
...
.modal-close{background:none;border:none;cursor:pointer;color:var(--text-muted);font-size:20px;padding:2px;line-height:1;transition:all 0.15s;border-radius:6px;width:28px;height:28px;display:flex;align-items:center;justify-content:center;}
...
export default function WaiterPanel() {
  ...
  // Auto-refresh active order statuses every 10s so waiters see "Preparing" / "Ready to Serve" updates
  useEffect(() => {
    if (view !== "order" || !selectedTable) return;
    const interval = setInterval(async () => {
      if (!selectedTable) return;
      try {
        const res = await API.get(`/orders/table/${selectedTable.id}/all`);
  ...
  const startTableTakeaway = async () => {
    if (!selectedTable) return;
    // If there's already an unsent draft, don't start another
    if (allRounds.some(r => r.order.status === "draft")) return;
    setRoundLoading(true);
    try {
  ...
  const addItem = async (menuItem, specialRequest) => {
    ...
    try {
      await API.post(`/orders/${dr.order.id}/items`, {
        menu_id: menuItem.id,
        quantity: 1,
        unit_price: special ? special.discounted_price : menuItem.price,
  ...
  const confirmLogout = async () => { try { await API.post("/auth/logout"); } catch {} logout(); navigate("/"); };
        ...
        {view === "tables" && (
          <div style={{ maxWidth:1200, margin:"0 auto", animation:"fadeIn 0.3s ease" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20, flexWrap:"wrap", gap:10 }}>
              <div>
                ...
                <p style={{ fontSize:13, color:"var(--text-muted)", marginTop:3 }}>
                  {tables.filter(t => t.status === "available").length} available · {tables.filter(t => t.status === "occupied").length} occupied · {tables.filter(t => t.status === "reserved").length} reserved
                </p>
              </div>
            ...
            {Object.keys(specials).length > 0 && (
              <div style={{ background:"linear-gradient(135deg,var(--accent-bg),rgba(245,158,11,0.04))", border:"1px solid var(--accent-border)", borderRadius:14, padding:"11px 16px", marginBottom:20, display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", animation:"slideDown 0.3s ease" }}>
                <span style={{ fontSize:18 }}>⭐</span>
                <span style={{ fontWeight:800, color:"var(--accent)", fontSize:13 }}>Today's Specials</span>
                {Object.values(specials).map((sp, i) => (
                  <span key={i} style={{ background:"var(--gradient-brand)", color:"#fff", borderRadius:20, padding:"3px 12px", fontSize:12, fontWeight:700, boxShadow:"0 2px 8px rgba(245,158,11,0.3)" }}>
            ...
            {loading ? (
              ...
              return sections.map(section => {
                ...
                return (
                  <div key={section} style={{ marginBottom: multiSection ? 28 : 0 }}>
                    {multiSection && (
                      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
                        <div style={{ height:1, flex:1, background:"var(--border)" }} />
                        <span style={{ fontSize:12, fontWeight:800, color:"var(--text-muted)", textTransform:"uppercase", letterSpacing:"0.1em", display:"flex", alignItems:"center", gap:6 }}>
                          📍 {section}
                          <span style={{ background:"var(--accent-bg)", border:"1px solid var(--accent-border)", color:"var(--accent)", borderRadius:20, padding:"1px 8px", fontSize:11 }}>{sectionTables.length}</span>
                        </span>
                        <div style={{ height:1, flex:1, background:"var(--border)" }} />
                      </div>
                    )}
                    <div className="table-grid">
                      {sectionTables.map((t, i) => (
                        <div key={t.id} className={`table-tile ${t.status}`} onClick={() => selectTable(t)} style={{ animationDelay:`${i*0.04}s` }}>
                          <div style={{ fontSize: t.table_label ? 26 : 22, fontWeight:900, lineHeight:1, letterSpacing:"-0.03em", color:"var(--text-primary)" }}>
                            {t.table_label || `T${t.table_number}`}
                          </div>
                          {t.table_label && <div style={{ fontSize:10, color:"var(--text-muted)", marginTop:2 }}>No. {t.table_number}</div>}
                          {t.table_section && t.table_section !== "Main" && !multiSection && (
            ...
            <div className="waiter-menu-col" style={{ minWidth:0 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:14 }}>
                {activeOrderType === "takeaway"
                  ...
                  : <div style={{ background:"var(--gradient-brand)", color:"#fff", borderRadius:10, padding:"7px 14px", fontWeight:800, fontSize:13 }}>
                      🪑 {selectedTable?.table_label || `Table ${selectedTable?.table_number}`}
                      {selectedTable?.table_section && selectedTable.table_section !== "Main" && (
                        <span style={{ opacity: 0.8, fontWeight: 600, fontSize: 11, marginLeft: 5 }}>· {selectedTable.table_section}</span>
                      )}
                    </div>
                }
                {allRounds.map((r, idx) =>
              ...
              {roundLoading ? (
                ...
                <div style={{ background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:16, padding:"40px 24px", textAlign:"center" }}>
                  ...
                  <div style={{ fontWeight:700, fontSize:16, marginBottom:8 }}>
                    {activeOrderType==="takeaway" ? "New Takeaway Order" : `${selectedTable?.table_label || `Table ${selectedTable?.table_number}`} is free`}
                  </div>
                  <p style={{ color:"var(--text-muted)", fontSize:13, marginBottom:20 }}>
                    {activeOrderType==="table" ? "Start a dine-in order or add a takeaway for this table." : "Items will appear in the summary panel."}
                  </p>
                  {activeOrderType==="table" && (
                    <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
                      <button className="btn btn-primary" onClick={startOrder}>🍽️ Start Table Order</button>
                      {/* <button className="btn btn-secondary" onClick={startTableTakeaway}>📦 Add Takeaway</button> */}
                    </div>
                  )}
                </div>
              ) : (
                <>
                  {draftRoundIdx === null && activeOrderType !== "takeaway" && (
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:11, color:"var(--text-muted)", marginTop:2, fontWeight:500 }}>
                      {activeOrderType==="takeaway" ? "📦 Takeaway"
                        : selectedTable ? `${selectedTable.table_label || `Table ${selectedTable.table_number}`} · ${allRounds.length} round${allRounds.length!==1?"s":""}` : "No active order"}
                    </div>
                  </div>
                            ...
                            : round.items.map(item => (
                              <OrderItemRow
                                ...
                                onUpdateRequest={async (req) => {
                                  try {
                                    await API.patch(`/orders/${round.order.id}/items/${item.id}/request`, { special_request: req });
                                    await refreshRound(idx);
                                  } catch {}
                                }}
                              />
                            ))
                ...
                {draftRound && draftRound.items.length > 0 && (
                  <div style={{ padding:"12px 14px", borderTop:"1px solid var(--border)" }}>
                    <button className="btn btn-primary" style={{ width:"100%", fontWeight:800, fontSize:14, padding:"13px 0", borderRadius:12, background:`linear-gradient(135deg,${ROUND_COLORS[draftRoundIdx%6]},${ROUND_COLORS[(draftRoundIdx+1)%6]})` }} onClick={() => setConfirmModal(true)}>
                      🚀 Send {draftRound?.order.order_type === "takeaway" ? "Takeaway Order" : getRoundLabel(draftRoundIdx)} to Kitchen / Bar
                    </button>
                ...
                <div>
                  ...
                  <div style={{ fontSize:12, color:"var(--text-muted)", marginTop:2 }}>
                    {draftRound?.order.order_type === "takeaway"
                      ? selectedTable ? `Takeaway — ${selectedTable.table_label || `Table ${selectedTable.table_number}`}` : "Takeaway order"
                      : selectedTable?.table_label || `Table ${selectedTable?.table_number}`}
                  </div>
                </div>
              </div>
            ...
            <div className="modal-body">
              ...
              <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:12, overflow:"hidden" }}>
                ...
                {draftRound.items.map((item, i) => (
                  <div key={i} style={{ borderBottom:i<draftRound.items.length-1?"1px solid var(--border)":"none" }}>
                    ...
                    {item.special_request && (
                      <div style={{ padding:"0 14px 10px", fontSize:12, color:"#d97706", fontWeight:600 }}>
...
function OrderItemRow({ item, isDraft, color, onRemove, onUpdateRequest }) {
  ...
  const QUICK = [
    "Less spicy","Extra spicy","No spice","Less oil","No onion","No garlic",
    "Less sauce","Extra sauce","No sauce","Less salt","Gluten free","Vegan",
    "Well done","Medium","Rare",
  ];

  const save = async () => {
    setSaving(true);
    await onUpdateRequest(reqText.trim());
    setSaving(false);
    setEditing(false);
  };

  return (
    <div className="order-item-row" style={{ flexDirection: "column", alignItems: "stretch", gap: 0 }}>
      {/* main row */}
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <div style={{ flex: 1 }}>
        ...
        <span style={{ fontWeight: 700, fontSize: 13, color: isDraft ? color : "var(--success)" }}>
        ...
        {isDraft && (
          <>
            <button
              ...
              style={{
                background: editing ? "var(--accent-bg)" : "none",
                border: "1px solid",
                borderColor: editing ? "var(--accent-border)" : "var(--border-md)",
                cursor: "pointer", color: editing ? "var(--accent)" : "var(--text-muted)",
      ...
      {!editing && item.special_request && (
        <div style={{
          ...
          borderRadius: 6, fontSize: 11, color: "#d97706", fontWeight: 600,
          display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6,
        }}>
          <span>⚠️ {item.special_request}</span>
          {isDraft && (
            <button onClick={() => setEditing(true)} style={{ background: "none", border: "none", cursor: "pointer", color: "#d97706", fontSize: 11, fontWeight: 700, padding: 0, fontFamily: "var(--font)" }}>
              Edit
            </button>
          )}
        </div>
      ...
      {editing && isDraft && (
        <div style={{
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 7 }}>
            {QUICK.map(q => (
              <button key={q} onClick={() => setReqText(t => t ? `${t}, ${q}` : q)} style={{
                padding: "3px 9px", borderRadius: 20, border: "1px solid var(--border-md)",
                background: reqText.includes(q) ? "var(--accent-bg)" : "var(--bg-elevated)",
                color: reqText.includes(q) ? "var(--accent)" : "var(--text-muted)",
                borderColor: reqText.includes(q) ? "var(--accent-border)" : "var(--border-md)",
          ...
          <textarea
            ...
            placeholder="Custom request, e.g. less spicy, no sauce..."
            rows={2}
            style={{
              width: "100%", padding: "7px 10px", background: "var(--bg-surface, var(--bg-card))",
              border: "1px solid var(--border-md)", borderRadius: 7,
              color: "var(--text-primary)", fontFamily: "var(--font)", fontSize: 12,
              resize: "vertical", outline: "none", boxSizing: "border-box",
            }}
          />
          <div style={{ display: "flex", gap: 6, marginTop: 6, justifyContent: "flex-end" }}>
            <button className="btn btn-ghost btn-sm" onClick={() => { setReqText(item.special_request || ""); setEditing(false); }}>Cancel</button>
            {reqText && <button className="btn btn-ghost btn-sm" onClick={() => setReqText("")} style={{ color: "var(--danger)" }}>Clear</button>}
