...
router.get("/", authMiddleware, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM tables WHERE restaurant_id=$1 ORDER BY COALESCE(table_section,'Main'), table_number", [rid]
    );
...
router.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      ...
      [table_number, capacity || 4, rid, table_label || null, table_section || "Main"]
    );
