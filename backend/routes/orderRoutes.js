  ...
  try {
    const result = await pool.query(`
      SELECT o.*,
        ...
        CASE
          -- Paid/closed/credit_pending: primary order total already holds grand total (incl. tax + discount)
          WHEN o.status = 'paid' THEN o.total
          WHEN o.status = 'closed' THEN o.total
          WHEN o.status = 'credit_pending' THEN o.total
          ...
          WHEN o.order_type = 'table' AND o.table_id IS NOT NULL THEN (
            ...
            WHERE o2.table_id = o.table_id
              ...
              AND o2.status NOT IN ('paid','closed','credit_pending')
          )
          ELSE o.total
...
router.get("/table/:tableId", authMiddleware, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT o.*, t.table_number, COALESCE(t.table_label,'') as table_label, COALESCE(t.table_section,'Main') as table_section FROM orders o
       LEFT JOIN tables t ON o.table_id = t.id
       WHERE o.table_id=$1 AND o.restaurant_id=$2 AND o.status NOT IN ('paid','closed','credit_pending')
...
router.post("/:orderId/items", authMiddleware, async (req, res) => {
  try {
    ...
    const { menu_id, quantity, unit_price, special_request } = req.body;
    ...
    const item = await pool.query(
      "INSERT INTO order_items (order_id, menu_id, quantity, price, special_request) VALUES ($1,$2,$3,$4,$5) RETURNING *",
      [orderId, menu_id, quantity, totalPrice, special_request || null]
    );
...
// UPDATE SPECIAL REQUEST on an order item
router.patch("/:orderId/items/:itemId/request", authMiddleware, async (req, res) => {
  try {
    const { orderId, itemId } = req.params;
    const { special_request } = req.body;
    const result = await pool.query(
      "UPDATE order_items SET special_request=$1 WHERE id=$2 AND order_id=$3 RETURNING *",
      [special_request || null, itemId, orderId]
    );
...
// CONFIRM ORDER (draft → pending, routed to kitchen/bar based on item categories)
router.put("/:id/confirm", authMiddleware, async (req, res) => {
  try {
    ...
    // Auto-detect destination: bar categories vs kitchen categories
    const items = await pool.query(
      `SELECT DISTINCT LOWER(m.category) as cat FROM order_items oi
       JOIN menu m ON oi.menu_id = m.id WHERE oi.order_id=$1`,
      [req.params.id]
    );
    const cats = items.rows.map(r => r.cat);
    // category CHECK in DB: food | drink | dessert | snack  — "drink" is the bar category
    const BAR_CATS = ["drink","drinks","beverage","beverages","bar","cocktail","mocktail","juice","beer","wine","alcohol"];
    const hasBar  = cats.some(c => BAR_CATS.includes(c));
    const hasFood = cats.some(c => !BAR_CATS.includes(c));
    const destination = hasBar && hasFood ? "mixed" : hasBar ? "bar" : "kitchen";
    const result = await pool.query(
      "UPDATE orders SET status='pending', destination=$2, updated_at=NOW() WHERE id=$1 RETURNING *",
  ...
  try {
    ...
    if (mainOrder.table_id) {
      ...
      if (mainOrder.status === "paid") {
        ...
        // — never include paid/closed/credit_pending orders from previous sessions.
        relatedOrders = await pool.query(
          `SELECT id FROM orders
           WHERE table_id=$1 AND restaurant_id=$2
             AND status NOT IN ('paid','closed','credit_pending')
           ORDER BY id ASC`,
          [mainOrder.table_id, mainOrder.restaurant_id]
        );
      }
